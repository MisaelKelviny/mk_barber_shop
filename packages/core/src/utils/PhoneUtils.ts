export default class PhoneUtils {
  static format(phone: string): string {
    ...
    return numbers.length <= 10
      ? this.substituteNumbers(numbers, "(xx) xxxx-xxxx")
      : this.substituteNumbers(numbers, "(xx) xxxxx-xxxx");
  }
  ...
  private static substituteNumbers(phone: string, ref: string): string {
    let formatted = phone
      .split("")
      .reduce((phone, number) => {
